import { filterValidSales, hasValidProjectName } from "../clean/filterValidSales";
import { SQM_TO_SQFT } from "../transform/pricePerSqft";
...
export function computeCityStats(rows) {
  ...
  const avgPriceSqftGlobal = withArea.length ? withArea.reduce((s, r) => s + parseFloat(r.TRANS_VALUE) / (parseFloat(r.PROCEDURE_AREA) * SQM_TO_SQFT), 0) / withArea.length : null;
  const areaMap = {};
  sales.forEach(r => {
  ...
  const topAreasByCount = Object.entries(areaMap).sort((a, b) => b[1].count - a[1].count).slice(0, 10).map(([name, d]) => ({
  ...
  const topAreasByValue = Object.entries(areaMap).sort((a, b) => b[1].value - a[1].value).slice(0, 10).map(([name, d]) => ({
    name: name.length > 22 ? name.slice(0, 20) + "…" : name,
    fullName: name,
    value: Math.round(d.value / 1_000_000),
    formattedValue: fmtM(d.value)
  }));
  const topAreasByPrice = Object.entries(areaMap).filter(([, d]) => d.priceCount >= 3).sort((a, b) => b[1].priceSum / b[1].priceCount - a[1].priceSum / a[1].priceCount).slice(0, 10).map(([name, d]) => ({
    name: name.length > 22 ? name.slice(0, 20) + "…" : name,
    fullName: name,
  ...
  sales.forEach(r => {
    ...
    if (!hasValidProjectName(proj)) return;
  ...
  const topProjects = Object.entries(projectMap).sort((a, b) => b[1].count - a[1].count).slice(0, 10).map(([name, d]) => ({
  ...
  const usageSplit = Object.entries(sales.reduce((acc, r) => {
  ...
  const propTypeSplit = Object.entries(sales.reduce((acc, r) => {
    const t = r.PROP_SB_TYPE_EN?.trim() || r.PROP_TYPE_EN?.trim() || "Other";
    acc[t] = (acc[t] || 0) + 1;
    return acc;
  }, {})).sort((a, b) => b[1] - a[1]).slice(0, 6).map(([name, value]) => ({
    name,
    value
  }));
  const roomsSplit = Object.entries(sales.reduce((acc, r) => {
