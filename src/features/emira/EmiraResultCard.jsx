...
import { LineChart, Line, BarChart, Bar, AreaChart, Area, PieChart, Pie, Legend, RadarChart, Radar, PolarGrid, PolarAngleAxis, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, PolarRadiusAxis, Cell } from "recharts";
...
export default function EmiraResultCard({
  ...
  const formatResult = text => {
    if (!text) return null;
    const HEADERS = ["CURRENT PRICE:", "1 YEAR FORECAST:", "3 YEAR FORECAST:", "5 YEAR FORECAST:", "GROSS YIELD:", "NET YIELD:", "RISK LEVEL:", "OVERALL OUTLOOK:", "HOT OR NOT:", "OVERSUPPLY RISK:", "MACRO RISKS:", "LIQUIDITY RISK:", "REGULATORY RISK:", "MARKET TIMING:", "TRANSACTION ACTIVITY:", "BUYER DEMAND:", "PRICE TREND:", "GOVERNMENT INITIATIVES:", "INFRASTRUCTURE:", "DEMAND FACTORS:", "SUPPLY PIPELINE:", "MARKET MOMENTUM:", "VERDICT:"];
    const ACCENT_HEADERS = ["CURRENT PRICE", "1 YEAR FORECAST", "3 YEAR FORECAST", "5 YEAR FORECAST", "CURRENT AVG RENT", "GROSS YIELD", "NET YIELD", "1 YEAR RENTAL INCOME", "3 YEAR RENTAL INCOME", "5 YEAR RENTAL INCOME", "RENTAL DEMAND OUTLOOK", "BEST PROPERTY TYPE", "RISK LEVEL", "OVERSUPPLY RISK", "MACRO RISKS", "LIQUIDITY RISK", "REGULATORY RISK", "MARKET TIMING", "RISK SUMMARY", "OVERALL OUTLOOK", "GOVERNMENT INITIATIVES", "INFRASTRUCTURE", "DEMAND FACTORS", "SUPPLY PIPELINE", "MARKET MOMENTUM", "VERDICT", "TRANSACTION ACTIVITY", "PRICE TREND", "OFF-PLAN VS READY", "BUYER DEMAND", "COMPARED TO DUBAI", "HOT OR NOT", "CONFIDENCE", "KEY DRIVERS", "DISCLAIMER"];
    const renderHighlighted = str => {
      ...
      const BOLD_PHRASES = ["GOLDEN VISA", "D33", "ECONOMIC AGENDA", "EMIRATES ROAD E611", "SHEIKH MOHAMMED BIN ZAYED ROAD E311", "2040 URBAN MASTER PLAN", "DUBAI METRO", "AL MAKTOUM AIRPORT"];
      return str.split(/(\*\*.*?\*\*)/g).flatMap((part, pi) => {
        if (part.startsWith("**") && part.endsWith("**")) return [<strong key={`b${pi}`} style={{
          fontWeight: 700
        ...
        while (remaining.length > 0 && safetyIdx++ < 200) {
          const redMatch = RED_WORDS.exec(remaining);
          RED_WORDS.lastIndex = 0;
          const greenMatch = GREEN_WORDS.exec(remaining);
          GREEN_WORDS.lastIndex = 0;
          let boldMatch = null;
          ...
          if (first === boldMatch) {
            segments.push(<strong key={`bold-${pi}-${first.index}`} style={{
              fontWeight: 700,
              color: "#000"
            }}>{first.text}</strong>);
            ...
            segments.push(<span key={`h${pi}-${first.index}`} style={{
              color: isRed ? "#e83f25" : "#22c55e",
              fontWeight: 600
            }}>
                {first[0]}
              </span>);
    ...
    return text.split("\n").map((line, i) => {
      ...
      if (!trimmed) return <div key={i} style={{
        height: 4
      }} />;
      ...
      if (matchedHeader && trimmed.includes(":")) {
        ...
        return <div key={i} style={{
          ...
          borderBottom: "1.5px solid #f0f0f0"
            ...
            color: isAccent ? "#e83f25" : "#939393",
            display: "block",
            marginBottom: 2
          }}>
              {label.trim()}
            </span>
            {value && <span style={isShort ? {
            fontFamily: "Fields Display, sans-serif",
            fontSize: 28,
            fontWeight: 700,
            color: "#000",
            lineHeight: 1.1
  ...
  const renderStatCards = () => {
    ...
    if (activeAnalysis === "PRICE_FORECAST") {
      ...
      if (current) cards.push({
        label: "Current Price",
        value: `AED ${current.toLocaleString()}`
      });
      if (yr5) cards.push({
        label: "5yr Target",
        value: `AED ${yr5.toLocaleString()}`
      });
      if (current && yr5) {
        const growth = Math.round((yr5 - current) / current * 100);
        if (growth !== null) cards.push({
      ...
      if (yr5Income) cards.push({
        label: "5yr Income",
        value: `AED ${yr5Income.toLocaleString()}`
      });
      ...
      if (demandMatch) cards.push({
        label: "Buyer Demand",
        value: demandMatch[1].toUpperCase()
      });
      if (trendMatch) cards.push({
        label: "Price Trend",
        value: trendMatch[1].toUpperCase()
      });
      if (hotMatch) cards.push({
        label: "Hot or Not",
        value: hotMatch[1].toUpperCase()
      });
      ...
      if (outlookMatch) {
        ...
        cards.push({
          label: "Outlook",
          value: ok.toUpperCase(),
          color: okColor
        });
      }
      if (momentumMatch) cards.push({
        label: "Market Momentum",
        value: momentumMatch[1].toUpperCase()
      });
    }
  ...
  const renderCharts = () => {
    ...
    if (activeAnalysis === "PRICE_FORECAST") {
      ...
      return <>
          {current && yr1 && yr3 && yr5 && <ChartCard title="Visual Summary">
              ...
              bottom: 5
                  ...
                  <XAxis dataKey="period" axisLine={false} tickLine={false} tick={{
                fontSize: 11,
                fontWeight: 600,
                fill: "#939393"
              }} dy={10} />
                ...
                fill: "#939393"
                  ...
                  <Tooltip {...ttStyle} />
                  <Line type="monotone" dataKey="price" stroke="#e83f25" strokeWidth={3} dot={{
                r: 6,
                fill: "#e83f25",
                strokeWidth: 2,
                stroke: "#fff"
          ...
          {areaData.length > 0 && <ChartCard title="Area Price Comparison">
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={areaData} margin={{
              top: 10,
              right: 20,
              left: 20,
              bottom: 44
                  ...
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                  <XAxis dataKey="area" axisLine={false} tickLine={false} tick={{
                fontSize: 10,
                fontWeight: 600,
                fill: "#555"
                ...
                fill: "#555"
                  ...
                  <Bar dataKey="price" radius={[3, 3, 0, 0]} barSize={36}>
                    {areaData.map((entry, index) => <Cell key={`cell-area-${index}`} fill={entry.fullName === selectedArea ? "#e83f25" : "#1e293b"} />)}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
            </ChartCard>}
      ...
      return <>
          <ChartCard title="Visual Summary">
            ...
            gap: 24
              ...
              {yr1Income && yr3Income && yr5Income && <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={[{
                period: "1 Year",
                income: yr1Income
              }, {
          ...
          {gross && net && gross > net && <ChartCard title="Yield Breakdown">
    ...
    if (activeAnalysis === "RISK_ASSESSMENT") {
      ...
      const data = [{
        risk: "Oversupply",
        value: parseRisk("OVERSUPPLY RISK:")
      }, {
        risk: "Macro",
        value: parseRisk("MACRO RISKS:")
      }, {
        risk: "Liquidity",
        value: parseRisk("LIQUIDITY RISK:")
      }, {
        risk: "Regulatory",
        value: parseRisk("REGULATORY RISK:")
      }, {
        risk: "Timing",
        value: timingMatch ? timingMatch[1].toLowerCase() === "good time" ? 1 : 2 : 2
      }];
          ...
          <ChartCard title="Risk Breakdown">
            <ResponsiveContainer width="100%" height={260}>
              <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data}>
                ...
                <PolarAngleAxis dataKey="risk" tick={{
                fontSize: 10,
                fontWeight: 700,
                fill: "#444"
              }} />
    ...
    if (activeAnalysis === "MARKET_PULSE") {
      const parsePulse = (label, rules) => {
        ...
        for (const rule of rules) if (rule.keywords.some(k => val.includes(k))) return rule.score;
        return null;
      };
      const transaction = parsePulse("TRANSACTION ACTIVITY:", [{
        keywords: ["high", "above average", "strong", "robust"],
        score: 80
      }, {
        keywords: ["moderate", "average", "balanced"],
        score: 55
      }, {
        keywords: ["low", "below", "marginal", "limited", "minimal"],
        score: 30
      }]);
      const demand = parsePulse("BUYER DEMAND:", [{
        keywords: ["strong", "high", "robust", "excellent"],
        score: 85
      }, {
        keywords: ["moderate", "average", "steady"],
        score: 55
      }, {
        keywords: ["weak", "low", "limited", "constrained"],
        score: 25
      }]);
      const trend = parsePulse("PRICE TREND:", [{
        keywords: ["upward", "rising", "increasing", "strong", "appreciat", "premium"],
        score: 75
      }, {
        keywords: ["stable", "steady", "flat"],
        score: 50
      }, {
        keywords: ["declining", "falling", "downward"],
        score: 25
      }]);
      const data = [{
        metric: "Transaction Activity",
        score: transaction ?? 0
      }, {
        metric: "Buyer Demand",
        score: demand ?? 0
      ...
      const dubaiAvg = stats?.totalTx && stats?.uniqueAreas?.length ? Math.round(stats.totalTx / stats.uniqueAreas.length) : null;
      return <>
          <ChartCard title="Visual Summary">
            <ResponsiveContainer width="100%" height={260}>
              ...
              bottom: 5
                ...
                <YAxis type="category" dataKey="metric" axisLine={false} tickLine={false} tick={{
                fontSize: 11,
                fontWeight: 700,
                fill: "#333"
              }} width={130} />
                ...
                <Bar dataKey="score" radius={[0, 4, 4, 0]} barSize={32}>
                  {data.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.score >= 70 ? "#22c55e" : entry.score >= 50 ? "#f59e0b" : "#ef4444"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>
          {areaStatEntry && dubaiAvg !== null && <ChartCard title="Transaction Volume vs Dubai Average">
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={[{
              name: (selectedArea || "Selected").slice(0, 14),
              value: areaStatEntry.count
            }, {
              name: "Dubai Avg",
              value: dubaiAvg
            }]} margin={{
    ...
    if (activeAnalysis === "GROWTH_DRIVERS") {
      ...
      const gdData = [{
        metric: "Government Initiatives",
        score: parseDriver("GOVERNMENT INITIATIVES:")
      }, {
        metric: "Infrastructure",
        score: parseDriver("INFRASTRUCTURE:")
      }, {
        metric: "Demand Factors",
        score: parseDriver("DEMAND FACTORS:")
      }];
      ...
      const phaseData = [{
        phase: "Short Term",
        score: scorePhase(/short[\s-]term.*?\b(strong|high|moderate|medium|weak|low)\b/i)
      }, {
        phase: "Mid Term",
        score: scorePhase(/mid[\s-]term.*?\b(strong|high|moderate|medium|weak|low)\b/i)
      }, {
        phase: "Long Term",
        score: scorePhase(/long[\s-]term.*?\b(strong|high|moderate|medium|weak|low)\b/i)
      }];
      return <>
          <ChartCard title="Growth Drivers">
            <ResponsiveContainer width="100%" height={200}>
              <BarChart layout="vertical" data={gdData} margin={{
              top: 5,
              right: 30,
              left: 60,
              bottom: 5
                ...
                <Bar dataKey="score" radius={[0, 4, 4, 0]} barSize={32}>
                  {gdData.map((entry, index) => <Cell key={`cell-gd-${index}`} fill={entry.score >= 70 ? "#22c55e" : entry.score >= 50 ? "#f59e0b" : "#ef4444"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </ChartCard>
          <ChartCard title="Growth Phase Outlook">
            <ResponsiveContainer width="100%" height={200}>
              <AreaChart data={phaseData} margin={{
              top: 10,
              right: 30,
              left: 0,
              bottom: 5
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f0f0f0" />
                <XAxis dataKey="phase" axisLine={false} tickLine={false} tick={{
                fontSize: 11,
                fontWeight: 600,
                fill: "#939393"
              }} />
                ...
                <Area type="monotone" dataKey="score" stroke="#e83f25" strokeWidth={2} fill="url(#growthGrad)" dot={{
                r: 5,
                fill: "#e83f25",
                stroke: "#fff",
                strokeWidth: 2
  ...
  if (!activeAnalysis) {
    return <motion.div key="empty" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} style={{
  ...
  return <motion.div key="result" initial={{
    ...
    overflow: "hidden"
      ...
      flexWrap: "wrap"
        ...
        <div>
          ...
          marginBottom: 8
            ...
            {isStreaming && <Loader2 size={12} className="spin" style={{
            color: "#e83f25"
          }} />}
            {isComplete && !isStreaming && <CheckCircle2 size={12} style={{
            color: "#e83f25"
          }} />}
            <span className="e-label" style={{
            color: isStreaming ? "#e83f25" : "#939393"
          }}>
        ...
        textAlign: "right"
          ...
          fontWeight: 700,
          color: "#e83f25"
        }}>{selectedArea || "General Dubai Market"}</div>
          <div style={{
          fontSize: 10,
          color: "#939393",
          marginTop: 2
      ...
      padding: "32px 36px"
        ...
        {streamError ? <div style={{
        background: "#fff8f7",
        border: "1.5px solid #fad0c9",
        padding: 32,
        display: "flex",
        flexDirection: "column",
        alignItems: "center",
        ...
        textAlign: "center"
          ...
          </div> : analysisResult ? <div className="fade-up">
            {renderStatCards()}
        ...
        gap: 16
          ...
          color: "#000"
            ...
            <p key={processingMsgIdx} className="fade-up" style={{
          color: "#939393",
          fontSize: 12,
      ...
      background: "#fafafa"
        ...
        <div>{isStreaming && <span className="e-label blink" style={{
          color: "#e83f25"
        }}>— Receiving secure transmission</span>}</div>
