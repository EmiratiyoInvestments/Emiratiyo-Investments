  ...
  return (
    <>
      ...
      <button
        ...
        className="emira-history-toggle"
        style={{
      ...
      >
        <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <History size={13} style={{ color: "#e83f25" }} />
          History {history.length > 0 && `(${history.length})`}
        </span>
        <ChevronDown
          size={14}
          style={{ color: "#939393", transform: isMobileOpen ? "rotate(180deg)" : "rotate(0deg)", transition: "transform 0.2s" }}
        />
      </button>

      {/* ── Panel ── */}
      <div
        className="emira-history-panel"
        style={{
          width: 240,
          flexShrink: 0,
          background: "#fff",
          ...
          {history.length === 0 ? (
            ...
            history.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  onClick={() => { loadHistoryItem(item.id); setMobileOpen?.(false); }}
