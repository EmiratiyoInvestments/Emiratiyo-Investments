...
export default function EmiraAuth({
  isAuthenticated,
  login,
  isLoggingOutRef
}) {
  ...
  const handleKeyDown = (index, e) => {
    if (e.key === "Backspace" && !codeChars[index] && index > 0) inputRefs.current[index - 1]?.focus();
  };
  return <AnimatePresence>
      {!isAuthenticated && !isLoggingOutRef.current && <>
        ...
        zIndex: 98
          ...
          <motion.div key="auth-modal" initial={{
        opacity: 0,
        scale: 0.96,
        y: 20
      }} animate={{
          ...
          boxShadow: "0 32px 80px rgba(0,0,0,0.10), 0 4px 20px rgba(0,0,0,0.06)"
            ...
            padding: "44px 44px 0"
              ...
              marginBottom: 24
                  ...
                  {codeChars.map((char, i) => <input key={i} ref={el => inputRefs.current[i] = el} type="password" inputMode="numeric" maxLength={1} value={char} onChange={e => handleDigitChange(i, e.target.value)} onKeyDown={e => handleKeyDown(i, e)} className="digit" />)}
                </div>
                <AnimatePresence>
