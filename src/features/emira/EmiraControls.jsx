    ...
    gap: 16
        ...
        flex: 1
          ...
          position: "relative"
            ...
            <select value={selectedArea} onChange={e => setSelectedArea(e.target.value)} className="e-select" disabled={csvLoading}>
        ...
        flex: 1
          ...
          <textarea value={additionalContext} onChange={e => setAdditionalContext(e.target.value)} placeholder="Property details, price points, investment goals..." rows={4} className="e-textarea" />
        </div>
      </div>      <div style={{
      flex: 1,
      ...
      overflow: "hidden"
        ...
        <div className="emira-btn-grid">
          ...
          return <button key={btn.id} onClick={() => isBusy ? stopAnalysis() : startAnalysis(btn.id)} disabled={isDisabled} className={`ab ${isActive ? "ab-active" : ""}`}>
                <div style={{
              width: 38,
              height: 38,
              flexShrink: 0,
              ...
              transition: "all 0.18s"
                  ...
                  {isBusy ? <Square size={14} fill="currentColor" style={{
                color: "#fff"
              }} /> : <btn.Icon size={15} />}
                </div>
                <div style={{
              ...
              background: "#e83f25"
                ...
                {isActive && !isBusy && isComplete && <CheckCircle2 size={14} style={{
              color: "#e83f25"
            }} />}
                {!isActive && !isBusy && <Play size={12} fill="currentColor" style={{
              color: "#ddd"
            }} />}
              </button>;
        })}
