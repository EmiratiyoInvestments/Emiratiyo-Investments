...
const Input = ({
  ...
  disabled
    ...
    <label htmlFor={id} className="block text-sm font-semibold text-[#000000] mb-2">
      {label}
      {!required && <span className="text-gray-400 ml-1">(optional)</span>}
    </label>

    <input type={type} id={id} name={id} placeholder={placeholder} required={required} value={value} onChange={onChange} disabled={disabled} className="w-full px-4 py-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-[#e83f25] focus:border-transparent outline-none transition-all text-[#000000]" />
  </div>;
