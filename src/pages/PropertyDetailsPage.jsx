...
const PropertyDetailPage = () => {
  ...
  const portableTextComponents = {
    block: {
      h1: ({children}) => <h1 className="text-3xl font-bold mb-4 text-black" style={{ fontFamily: 'var(--font-display)' }}>{children}</h1>,
      h2: ({children}) => <h2 className="text-2xl font-bold mb-3 text-black" style={{ fontFamily: 'var(--font-display)' }}>{children}</h2>,
      h3: ({children}) => <h3 className="text-xl font-bold mb-3 text-black" style={{ fontFamily: 'var(--font-display)' }}>{children}</h3>,
      normal: ({children}) => <p className="mb-4 text-gray-600 leading-relaxed" style={{ fontFamily: 'var(--font-body)' }}>{children}</p>,
      blockquote: ({children}) => <blockquote className="border-l-4 border-[#e83f25] pl-4 italic my-4 text-gray-600">{children}</blockquote>,
    },
    marks: {
      strong: ({children}) => <strong className="font-bold text-black">{children}</strong>,
      em: ({children}) => <em className="italic">{children}</em>,
      link: ({children, value}) => (
        <a href={value.href} className="text-[#e83f25] hover:underline" target="_blank" rel="noopener noreferrer">
          {children}
        </a>
      ),
    },
    list: {
      bullet: ({children}) => <ul className="list-disc list-inside mb-4 space-y-2 text-gray-600" style={{ fontFamily: 'var(--font-body)' }}>{children}</ul>,
      number: ({children}) => <ol className="list-decimal list-inside mb-4 space-y-2 text-gray-600" style={{ fontFamily: 'var(--font-body)' }}>{children}</ol>,
    },
    listItem: {
    ...
    types: {
      image: ({value}) => (
        <div className="my-6">
          ...
          />
          {value.caption && (
            <p className="text-sm text-gray-500 text-center mt-2" style={{ fontFamily: 'var(--font-body)' }}>
              {value.caption}
            </p>
          )}
        </div>
      ...
      <>
        ...
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            <h2 className="text-2xl font-bold text-black mb-4" style={{ fontFamily: 'var(--font-display)' }}>
              Property Not Found
            </h2>
            <Link to="/properties" className="text-[#e83f25] hover:underline" style={{ fontFamily: 'var(--font-body)' }}>
              ← Back to Properties
            </Link>
          </div>
        </div>
          ...
          {allImages.length > 0 && (
            <div className="mb-12">
              <div className="relative h-96 lg:h-[500px] rounded-2xl overflow-hidden bg-gray-100">
                ...
                {/* Image Counter */}
                {allImages.length > 1 && (
                  <div className="absolute top-4 right-4 bg-black/60 text-white px-3 py-1 rounded-full text-sm" style={{ fontFamily: 'var(--font-body)' }}>
                  ...
                  {allImages.slice(0, 6).map((image, index) => (
                    <button
                      ...
                      className={`relative h-20 rounded-lg overflow-hidden ${
                        index === currentImageIndex ? 'ring-2 ring-[#e83f25]' : 'opacity-60 hover:opacity-100'
                      } transition-all`}
                    >
                      <img
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 p-6 bg-[#f7f7f7] rounded-xl">
                {property.bedrooms && (
                  <div className="text-center">
                    ...
                    <p className="text-2xl font-bold text-black" style={{ fontFamily: 'var(--font-display)' }}>
                      {property.bedrooms}
                    </p>
                ...
                {property.completionStatus && (
                  <div className="text-center">
                    ...
                    <p className="text-lg font-bold text-black capitalize" style={{ fontFamily: 'var(--font-display)' }}>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                ...
                <div className="bg-[#f7f7f7] p-6 rounded-xl">
                  ...
                  <button
                    className="w-full bg-[#e83f25] text-white py-3 rounded-lg font-semibold hover:bg-[#d63620] transition-colors"
                    style={{ fontFamily: 'var(--font-body)' }}
                  >
                    Contact Agent
                  </button>
                  ...
                  <div className="space-y-3 text-sm">
                    {property.propertyId && (
                      <div className="flex justify-between">
                        <span className="text-gray-500" style={{ fontFamily: 'var(--font-body)' }}>Property ID</span>
                        <span className="font-medium text-black" style={{ fontFamily: 'var(--font-body)' }}>{property.propertyId}</span>
                      </div>
                    )}
                    {property.developer && (
                      <div className="flex justify-between">
                        <span className="text-gray-500" style={{ fontFamily: 'var(--font-body)' }}>Developer</span>
                        <span className="font-medium text-black" style={{ fontFamily: 'var(--font-body)' }}>{property.developer}</span>
                      </div>
                    )}
                    {property.handoverDate && (
                      <div className="flex justify-between">
                        <span className="text-gray-500" style={{ fontFamily: 'var(--font-body)' }}>Handover Date</span>
                        <span className="font-medium text-black" style={{ fontFamily: 'var(--font-body)' }}>{formatDate(property.handoverDate)}</span>
                      </div>
                    )}
                    {property.publishedAt && (
                      <div className="flex justify-between">
                        <span className="text-gray-500" style={{ fontFamily: 'var(--font-body)' }}>Listed On</span>
                        <span className="font-medium text-black" style={{ fontFamily: 'var(--font-body)' }}>{formatDate(property.publishedAt)}</span>
                      </div>
                    )}
                    {property.location?.fullAddress && (
                      <div className="pt-3 border-t border-gray-200">
                        <p className="text-gray-500 mb-1" style={{ fontFamily: 'var(--font-body)' }}>Address</p>
                        <p className="text-black" style={{ fontFamily: 'var(--font-body)' }}>{property.location.fullAddress}</p>
                      </div>
                    )}
                  </div>
                </div>
