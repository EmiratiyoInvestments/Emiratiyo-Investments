...
const FAQItem = ({
  ...
  return <div className={`mb-4 rounded-2xl overflow-hidden transition-all duration-300 ${isOpen ? "bg-white shadow-lg" : "bg-[#f7f7f7]"}`}>
            <button onClick={onClick} aria-expanded={isOpen} className="w-full px-8 py-6 flex items-center justify-between text-left focus:outline-none">
                <span className={`text-lg font-bold transition-colors duration-300 ${isOpen ? "text-[#e83f25]" : "text-[#000000]"}`} style={{
        fontFamily: 'var(--font-body)'
      }}>
                    {question}
                </span>
                <div className={`transition-transform duration-300 ${isOpen ? "rotate-45" : "rotate-0"}`}>
                    <Plus className={`w-6 h-6 transition-colors duration-300 ${isOpen ? "text-[#e83f25]" : "text-[#000000]"}`} />
                </div>
            </button>
        ...
        ease: "easeInOut"
                        ...
                        <div className="px-8 pb-8">
                            <p className="text-[#939393] leading-relaxed" style={{
            fontFamily: 'var(--font-body)'
          }}>
                                {answer}
                            </p>
  ...
  return <section className="py-24 bg-white relative">
            <div className="max-w-4xl mx-auto px-6">
                <div className="text-center mb-16">
                    <h2 className="text-4xl md:text-5xl font-bold text-[#000000]" style={{
          fontFamily: 'var(--font-display)'
        }}>
                        Any questions? <span className="text-[#e83f25]" style={{
            fontFamily: 'var(--font-body)'
          }}>We got you.</span>
                    </h2>
                </div>
                ...
                <div className="space-y-4">
                    {faqData.map((item, index) => <FAQItem key={index} question={item.question} answer={item.answer} isOpen={openIndex === index} onClick={() => handleToggle(index)} />)}
                </div>
            </div>
        </section>;
};
