...
const Testimonials = () => {
  ...
  const TestimonialCard = ({
    testimonial
      ...
      <div className="flex gap-1 mb-4">
        {[...Array(testimonial.rating)].map((_, index) => <svg key={index} className="w-5 h-5 fill-[#e83f25]" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
        <img className="w-12 h-12 rounded-full object-cover ring-2 ring-gray-100" src={testimonial.image} alt={testimonial.name} />
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <p className="font-bold text-black text-sm">{testimonial.name}</p>
            <svg className="w-4 h-4 fill-[#e83f25]" viewBox="0 0 12 12" xmlns="http://www.w3.org/2000/svg">
  ...
  return <section className="py-16 sm:py-20 lg:py-28 bg-gray-50 overflow-hidden">
      ...
      <div className="marquee-row w-full mx-auto overflow-hidden relative mb-8">
        ...
        <div className="marquee-inner flex transform-gpu min-w-[200%] py-4">
          {[...testimonialsData, ...testimonialsData].map((testimonial, index) => <TestimonialCard key={`row1-${index}`} testimonial={testimonial} />)}
        </div>
      ...
      <div className="marquee-row w-full mx-auto overflow-hidden relative">
        ...
        <div className="marquee-inner marquee-reverse flex transform-gpu min-w-[200%] py-4">
          {[...testimonialsData, ...testimonialsData].map((testimonial, index) => <TestimonialCard key={`row2-${index}`} testimonial={testimonial} />)}
        </div>
        <div className="absolute right-0 top-0 h-full w-20 md:w-32 z-10 pointer-events-none bg-gradient-to-l from-gray-50 to-transparent"></div>
      </div>
