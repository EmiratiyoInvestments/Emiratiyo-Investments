...
const getLabel = a => (a.alias || a.name || a.commNum || "").replace(/_/g, " ");
const shortLabel = a => {
  const l = getLabel(a);
  const words = l.split(" ");
...
export default function AutoInsights() {
  ...
  const areas = Object.entries(areaData).map(([commNum, d]) => d && typeof d === "object" ? {
    commNum,
    ...d
  } : null).filter(Boolean);
  const topSale = [...areas].filter(a => a.sale_apartment).sort((a, b) => b.sale_apartment - a.sale_apartment).slice(0, 10).map(a => ({
    name: shortLabel(a),
    fullName: getLabel(a),
    value: a.sale_apartment,
    formattedValue: `AED ${fmt(a.sale_apartment)}/sqft`
  }));
  const affordable = [...areas].filter(a => a.sale_apartment).sort((a, b) => a.sale_apartment - b.sale_apartment).slice(0, 5);
  const topByTxCount = [...areas].filter(a => (a._txCount || 0) > 0).sort((a, b) => (b._txCount || 0) - (a._txCount || 0)).slice(0, 10).map(a => ({
    name: shortLabel(a),
    fullName: getLabel(a),
    value: a._txCount || 0,
  ...
  const saleColors = ["#e83f25", "#ef4444", "#fb923c", "#fbbf24", "#fde047", "#fde047", "#bef264", "#86efac", "#4ade80", "#22c55e"];
  return <div style={{
    fontFamily: "var(--font-body)"
  }}>
      <div style={{
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-7">
        ...
        padding: 20
          ...
          <div className="h-[320px] sm:h-[400px] lg:h-[280px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topSale} layout="vertical" margin={{
              left: 0,
              right: 30,
              top: 0,
              bottom: 0
                ...
                <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                  {topSale.map((_, i) => <Cell key={i} fill={saleColors[i] ?? "#e5e7eb"} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        padding: 20
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={topByTxCount} layout="vertical" margin={{
            left: 0,
            right: 16,
            top: 0,
            bottom: 0
              ...
              <Tooltip content={<CustomTooltip />} />
              <Bar dataKey="value" radius={[0, 4, 4, 0]}>
                {topByTxCount.map((_, i) => <Cell key={i} fill={["#e83f25", "#3b82f6", "#22c55e", "#8b5cf6", "#f59e0b", "#06b6d4", "#ec4899", "#84cc16", "#f97316", "#6366f1"][i] ?? "#e5e7eb"} />)}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      ...
      gap: 14
        ...
        overflow: "hidden"
          ...
          gap: 8
            ...
            <DollarSign size={18} strokeWidth={2.5} style={{
            color: "#22c55e"
          }} />
            <div>
              <p style={{
          ...
          fontSize: 13
            ...
            <tbody>
              {affordable.map((a, i) => <tr key={i} style={{
