...
export default function TransactionInsights() {
  const { rows, stats: computed, loading, error } = useCsvData();
  ...
  const TABS = [
    ...
    { key: "browse",   label: "Browse Data",  Icon: Home },
  ];
  ...
  return (
    <div style={{ fontFamily: "var(--font-body)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 14, marginBottom: 28 }}>
        ...
        {computed.avgPriceSqftGlobal && (
          <StatCard icon={Ruler} label="Avg Price/sqft" value={`AED ${fmt(computed.avgPriceSqftGlobal)}`} sub="Sales transactions" accent="#f59e0b" />
        )}
      </div>
