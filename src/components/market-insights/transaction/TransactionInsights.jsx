...
const COLORS = ["#e83f25", "#3b82f6", "#22c55e", "#8b5cf6", "#f59e0b", "#06b6d4", "#ec4899", "#84cc16", "#f97316", "#6366f1"];
const PAGE_SIZE = 20;
const CustomTooltip = ({
  active,
  payload,
...
export default function TransactionInsights() {
  ...
  const browseData = useMemo(() => {
    ...
    if (search.trim()) {
      ...
      data = data.filter(r => r.AREA_EN?.toLowerCase().includes(q) || r.PROJECT_EN?.toLowerCase().includes(q) || r.MASTER_PROJECT_EN?.toLowerCase().includes(q) || r.TRANSACTION_NUMBER?.toLowerCase().includes(q));
    }
    ...
    data = [...data].sort((a, b) => {
      ...
      if (sortKey === "date") {
        ...
        const sqmA = parseFloat(a.PROCEDURE_AREA);
        const sqmB = parseFloat(b.PROCEDURE_AREA);
  ...
  if (error || !computed) return <div style={{
    background: "#fef2f2",
    ...
    fontFamily: "var(--font-body)"
      ...
      fontWeight: 700,
      color: "#991b1b"
    }}>Failed to load transaction data</p>
      <p style={{
      fontSize: 12,
      color: "#7f1d1d",
      marginTop: 6
    }}>Make sure <code>/public/data/transaction-26.csv</code> exists.</p>
    </div>;
  const TABS = [{
    key: "areas",
    label: "Top Areas",
  ...
  const tabData = {
    areas: {
      data: computed.topAreasByCount,
      color: "#e83f25",
      label: "Transactions"
    },
    value: {
      data: computed.topAreasByValue,
      color: "#3b82f6",
      label: "AED Millions"
    },
    price: {
      data: computed.topAreasByPrice,
      color: "#8b5cf6",
      label: "AED/sqft"
    },
    projects: {
      data: computed.topProjects,
      color: "#22c55e",
      label: "Deals"
    }
  };
  ...
  return <div style={{
    fontFamily: "var(--font-body)"
      ...
      marginBottom: 28
        ...
        {computed.avgPriceSqftGlobal && <StatCard icon={Ruler} label="Avg Price/sqft" value={`AED ${fmt(computed.avgPriceSqftGlobal)}`} sub="Sales transactions" accent="#f59e0b" />}
      </div>
      <div style={{
      background: "#fff",
      border: "1px solid #e5e7eb",
      ...
      marginBottom: 28
        ...
        borderBottom: "1px solid #f1f5f9"
          ...
          flexWrap: "wrap"
            ...
            return <button key={t.key} onClick={() => {
              ...
              background: activeTab === t.key ? "#e83f25" : "#f1f5f9",
        ...
        {activeTab !== "browse" && active && <div style={{
        padding: 20
      }}>
            <p style={{
          fontSize: 11,
          ...
          marginBottom: 16
            ...
            <ResponsiveContainer width="100%" height={320}>
              <BarChart data={active.data} layout="vertical" margin={{
            left: 0,
            right: 20,
            top: 0,
            bottom: 0
        ...
        padding: 20
          ...
          marginBottom: 16
            ...
            setPage(1);
          }} placeholder="Search area, project, transaction number…" style={{
            flex: 1,
            minWidth: 220,
            border: "1px solid #e5e7eb",
            borderRadius: 10,
                ...
                return <tr key={i} style={{
                  background: i % 2 === 0 ? "#fff" : "#fafafa"
                    ...
                    color: "#475569"
                        ...
                        <td style={tdStyle}>
                          <Badge label={isOffPlan ? "Off-Plan" : "Ready"} color={isOffPlan ? "#0369a1" : "#15803d"} bg={isOffPlan ? "#eff6ff" : "#f0fdf4"} />
                        </td>
      ...
      marginBottom: 28
        ...
        sub: "By number of sales",
        data: computed.offPlanVsReady
      }, {
        title: "Usage Breakdown",
        sub: "Residential vs Commercial",
        data: computed.usageSplit
      }, {
        title: "Property Type",
        sub: "Flat, Villa, Townhouse etc.",
        data: computed.propTypeSplit
      }, {
        title: "Unit Size",
        sub: "Studio, 1BR, 2BR, 3BR etc.",
        data: computed.roomsSplit
      }].map(({
        title,
        sub,
        data
