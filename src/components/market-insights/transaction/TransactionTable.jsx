...
import { SQM_TO_SQFT, filterValidSales, useTransactionData } from "../../../features/market-intelligence";
const PAGE_SIZE = 15;
...
const SortArrow = ({
  ...
  if (sortKey !== k) return <span style={{
    color: "#cbd5e1",
    marginLeft: 4
  }}>↕</span>;
  return <span style={{
...
const DEFAULT_VISIBLE = new Set(["txNum", "date", "area", "project", "type", "freehold", "usage", "propType", "rooms", "parking", "value", "priceSqft", "sizeSqft", "buyers", "metro"]);
export default function TransactionTable() {
  const { rows, loading, error } = useTransactionData();
  ...
  const salesRows = useMemo(() => filterValidSales(rows), [rows]);
  const areaOptions = useMemo(() => [...new Set(salesRows.map(r => r.AREA_EN?.trim()).filter(Boolean))].sort(), [salesRows]);
  const filtered = useMemo(() => {
    let data = [...salesRows];
    if (search.trim()) {
      const q = search.trim().toLowerCase();
      data = data.filter(r => r.AREA_EN?.toLowerCase().includes(q) || r.PROJECT_EN?.toLowerCase().includes(q) || r.MASTER_PROJECT_EN?.toLowerCase().includes(q) || r.TRANSACTION_NUMBER?.toLowerCase().includes(q) || r.NEAREST_METRO_EN?.toLowerCase().includes(q) || r.NEAREST_LANDMARK_EN?.toLowerCase().includes(q));
    }
    ...
    data.sort((a, b) => {
      ...
      if (sortKey === "date") {
        ...
        va = parseFloat(a.TRANS_VALUE) || 0;
        vb = parseFloat(b.TRANS_VALUE) || 0;
      } else if (sortKey === "priceSqft") {
        const sA = parseFloat(a.PROCEDURE_AREA),
          sB = parseFloat(b.PROCEDURE_AREA);
        va = sA > 0 ? (parseFloat(a.TRANS_VALUE) || 0) / (sA * SQM_TO_SQFT) : 0;
        vb = sB > 0 ? (parseFloat(b.TRANS_VALUE) || 0) / (sB * SQM_TO_SQFT) : 0;
      } else if (sortKey === "sizeSqft") {
        va = parseFloat(a.PROCEDURE_AREA) || 0;
        vb = parseFloat(b.PROCEDURE_AREA) || 0;
      }
  ...
  if (error) return <div style={{
    ...
    fontFamily: "var(--font-body)"
      ...
      fontWeight: 700,
      color: "#991b1b"
    }}>Failed to load: {error?.message ?? String(error)}</p>
    </div>;
    ...
    switch (col.key) {
      case "txNum":
        return <td key={col.key} style={{
          ...
          whiteSpace: "nowrap"
        }}>{r.TRANSACTION_NUMBER?.replace("-2026", "") || "—"}</td>;
      case "date":
        return <td key={col.key} style={{
          ...tdBase,
          whiteSpace: "nowrap"
      ...
      case "masterProject":
        return <td key={col.key} style={{
          ...
          color: "#64748b",
          minWidth: 140
        }}>{master.length > 28 ? master.slice(0, 26) + "…" : master}</td>;
      case "project":
        return <td key={col.key} style={{
          ...tdBase,
          color: "#475569",
          minWidth: 160
        }}>{project.length > 30 ? project.slice(0, 28) + "…" : project}</td>;
      case "type":
      ...
      case "priceSqft":
        return <td key={col.key} style={{
          ...
          color: "#7c3aed",
          whiteSpace: "nowrap"
        }}>{priceSqft ? `${fmt(priceSqft)}` : "—"}</td>;
      case "sizeSqft":
        return <td key={col.key} style={{
          ...tdBase,
          whiteSpace: "nowrap"
        }}>{sqft ? sqft.toLocaleString() : "—"}</td>;
      case "actualSqft":
        return <td key={col.key} style={{
          ...tdBase,
          whiteSpace: "nowrap"
        }}>{actualSqft ? actualSqft.toLocaleString() : "—"}</td>;
      case "buyers":
        return <td key={col.key} style={{
          ...tdBase,
          textAlign: "center"
        }}>{r.TOTAL_BUYER?.trim() || "—"}</td>;
      case "sellers":
        return <td key={col.key} style={{
          ...tdBase,
          textAlign: "center"
        }}>{r.TOTAL_SELLER?.trim() || "—"}</td>;
      case "metro":
        return <td key={col.key} style={{
          ...tdBase,
          fontSize: 12,
  ...
  return <div style={{
    fontFamily: "var(--font-body)"
      ...
      alignItems: "center"
        ...
        setPage(1);
      }} placeholder="Search area, project, transaction #, landmark…" style={{
        flex: 1,
        minWidth: 240,
        border: "1px solid #e5e7eb",
        borderRadius: 10,
        ...
        <select value={filterArea} onChange={e => {
        ...
        <select value={filterUsage} onChange={e => {
        ...
        <select value={filterFreehold} onChange={e => {
        setFilterFreehold(e.target.value);
        setPage(1);
      }} style={{
        border: "1px solid #e5e7eb",
        borderRadius: 10,
        ...
        <button onClick={exportToCsv} disabled={!filtered.length} style={{
        border: "1px solid #e5e7eb",
        borderRadius: 10,
        padding: "9px 14px",
        fontSize: 13,
        ...
        position: "relative"
          ...
          fontWeight: 700,
          fontFamily: "var(--font-body)",
          background: showColPicker ? "#0f172a" : "#fff",
          ...
          gap: "6px 16px"
            ...
            marginBottom: 4
              ...
              {ALL_COLUMNS.map(col => <label key={col.key} style={{
            display: "flex",
            alignItems: "center",
            gap: 6,
            fontSize: 12,
            ...
            whiteSpace: "nowrap"
                  ...
                  <input type="checkbox" checked={visibleCols.has(col.key)} onChange={() => toggleCol(col.key)} style={{
              accentColor: "#e83f25",
              cursor: "pointer"
            }} />
                  {col.label}
        ...
        {hasActiveFilters && <button onClick={resetFilters} style={{
        border: "1px solid #fecaca",
        borderRadius: 10,
        padding: "9px 14px",
        fontSize: 12,
      ...
      overflow: "hidden"
        ...
        overflowX: "auto"
          ...
          borderCollapse: "collapse"
            ...
            <thead>
              <tr>
                {visibleColDefs.map(col => <th key={col.key} style={thStyle(col)} onClick={() => col.sortable && handleSort(col.key)}>
                    {col.label}
            ...
            <tbody>
              ...
              {paged.length === 0 && <tr>
                  <td colSpan={visibleColDefs.length} style={{
                padding: "48px 0",
                textAlign: "center",
                color: "#94a3b8",
                fontSize: 14
