...
// Custom dropdown component (avoids native <select> for consistent styling)
const Dropdown = ({ icon: Icon, label, value, options, onChange, id }) => {
      ...
      <button
        ...
        className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 border ${
          value !== options[0]?.value
            ? "bg-[#e83f25]/5 border-[#e83f25]/20 text-[#e83f25]"
            : "bg-[#f7f7f7] border-transparent text-gray-600 hover:bg-gray-100"
        }`}
          ...
          {options.map((option) => (
            <button
              ...
              className={`w-full text-left px-4 py-2.5 text-sm transition-colors ${
                value === option.value
                  ? "bg-[#e83f25]/5 text-[#e83f25] font-semibold"
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {hasActiveFilters && (
          <button
            onClick={onClearAll}
            className="flex items-center gap-1.5 px-3 py-3 rounded-xl text-sm font-medium text-gray-500 hover:text-[#e83f25] hover:bg-[#e83f25]/5 transition-all duration-200"
            style={{ fontFamily: "var(--font-body)" }}
