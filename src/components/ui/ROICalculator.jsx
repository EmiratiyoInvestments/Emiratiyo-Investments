...
function calculateROI(inputs) {
  ...
  const emi = loanAmount > 0 ? loanAmount * monthlyRate / (1 - Math.pow(1 + monthlyRate, -totalMonths)) : 0;
  ...
  const remainingLoanBalance = loanAmount > 0 ? loanAmount * Math.pow(1 + monthlyRate, totalMonths) - emi * (Math.pow(1 + monthlyRate, totalMonths) - 1) / monthlyRate : 0;
...
function InputField({
  ...
  return <div style={{
    zIndex: 1000
      ...
      <label className="text-sm font-semibold text-black mb-1 block" style={{
      fontFamily: 'var(--font-body)'
    }}>
        {label}
      </label>
      <div className="relative">
        {prefix && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-xs text-gray-400 font-medium">
            {prefix}
          </span>}
        <input type="number" name={name} value={value} onChange={onChange} placeholder={placeholder} className={`w-full border border-gray-200 rounded-lg py-3 text-sm focus:outline-none focus:border-[#e83f25] transition-colors ${prefix ? 'pl-12 pr-4' : 'px-4'}`} style={{
        fontFamily: 'var(--font-body)'
      }} />
      </div>
    </div>;
...
function SliderField({
  ...
  return <div>
      <label className="text-sm font-semibold text-black mb-1 flex items-center gap-2" style={{
      fontFamily: 'var(--font-body)'
    }}>
        {label}:
      ...
      <input type="range" name={name} value={value} onChange={onChange} min={min} max={max} step={step} className="w-full accent-[#e83f25]" />
      <div className="flex justify-between text-xs text-gray-400 mt-1" style={{
      fontFamily: 'var(--font-body)'
    }}>
        <span>{min}{unit}</span>
        <span>{max}{unit}</span>
...
function StatCard({
  ...
  return <div className={`rounded-xl p-4 ${highlight ? 'bg-[#e83f25] text-white' : 'bg-[#f7f7f7]'}`}>
      <div className="flex items-center gap-2 mb-1">
        <Icon className={`w-4 h-4 ${highlight ? 'text-white/70' : 'text-[#e83f25]'}`} />
        <p className={`text-xs ${highlight ? 'text-white/80' : 'text-gray-500'}`} style={{
        fontFamily: 'var(--font-body)'
      }}>
          {label}
        </p>
      </div>
      <p className={`text-lg font-bold ${highlight ? 'text-white' : 'text-black'}`} style={{
...
const ROICalculator = () => {
  ...
  return <>      <button onClick={() => setIsOpen(true)} className="fixed bottom-6 right-6 z-[9999] bg-[#e83f25] text-white rounded-full shadow-2xl hover:bg-[#d63620] transition-all hover:scale-110 flex items-center gap-2 px-4 py-3" style={{
      fontFamily: 'var(--font-body)'
    }}>
        ...
        {isOpen && <>
          ...
          opacity: 0
        }} onClick={() => setIsOpen(false)} className="fixed inset-0  z-[10000] bg-black/50" />
          ...
          damping: 25,
          stiffness: 300
        }} className="fixed right-0 top-0 h-full z-[1001] bg-white shadow-2xl overflow-y-auto" style={{
          width: '440px',
          maxWidth: '100vw',
          zIndex: 10001
              ...
              <div className="p-6 space-y-6">                <section className="space-y-4">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider" style={{
                fontFamily: 'var(--font-body)'
              }}>
                    Property Details
                  </h3>
                ...
                fontFamily: 'var(--font-body)'
                  ...
                  <SliderField label="Down Payment" name="downPaymentPct" value={inputs.downPaymentPct} onChange={handleChange} min={10} max={100} step={5} extra={results ? `= AED ${fmt(results.downPayment)}` : ''} />
                  <SliderField label="Interest Rate" name="interestRate" value={inputs.interestRate} onChange={handleChange} min={1} max={15} step={0.25} />
                  <SliderField label="Loan Term" name="loanYears" value={inputs.loanYears} onChange={handleChange} min={5} max={25} step={1} unit=" yrs" extra={results ? `EMI = AED ${fmt(results.emi)}/mo` : ''} />
                </section>                <section>
                  <button onClick={() => setShowAdv(!showAdv)} className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider w-full" style={{
                fontFamily: 'var(--font-body)'
              }}>
                  ...
                  {showAdv && <div className="mt-4 space-y-4 bg-gray-50 rounded-xl p-4">
                ...
                </section>                {results ? <section className="space-y-3">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider" style={{
                fontFamily: 'var(--font-body)'
              }}>
                      Your Results
                      ...
                      <div>
                        <p className="text-white/80 text-xs mb-1" style={{
                    fontFamily: 'var(--font-body)'
                  }}>
                          Cash-on-Cash Return
                        </p>
                    ...
                    fontFamily: 'var(--font-display)'
                        ...
                        <p className="text-white/70 text-xs mt-1" style={{
                    fontFamily: 'var(--font-body)'
                  }}>
                      ...
                      <div>
                        <p className="text-white/60 text-xs mb-1" style={{
                    fontFamily: 'var(--font-body)'
                  }}>
                    ...
                    fontFamily: 'var(--font-display)'
                        ...
                        <p className="text-white/50 text-xs mt-1" style={{
                    fontFamily: 'var(--font-body)'
                  }}>
                    ...
                    </div>                    <p className="text-xs text-gray-400 text-center" style={{
                fontFamily: 'var(--font-body)'
              }}>
                  ...
                  fontFamily: 'var(--font-body)'
                      ...
                      <a href="/contact" className="block w-full bg-[#e83f25] text-white py-3 rounded-lg text-sm font-bold hover:bg-[#d63620] transition-colors" style={{
                  fontFamily: 'var(--font-body)'
                }}>
                        Book Free Consultation
                      </a>
                    </div>
                  </section> : <div className="text-center py-8 text-gray-400">
